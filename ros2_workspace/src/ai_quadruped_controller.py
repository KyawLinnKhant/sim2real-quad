...
class AIQuadrupedController(Node):
    def __init__(self):
        ...
        # Confirm node is up and policy path is registered
        self.get_logger().info("🤖 AI Quadruped Controller ready.")
    ...
    def load_policy(self):
        ...
        try:
            ...
            # Confirm successful policy load
            self.get_logger().info("✅ Policy loaded and set to eval mode.")
    ...
    def cmd_vel_callback(self, msg):
        ...
        # Log incoming velocity command values
        self.get_logger().info(f"🎮 Vel cmd recv — vx={self.cmd_linear_x:.2f}, vy={self.cmd_linear_y:.2f}, wz={self.cmd_angular_z:.2f}")
    ...
    def build_observation(self):
        ...
        # Periodic shape check for debugging — remove after validation
        if self.step_count % 100 == 0:  # Print every 100 steps
            self.get_logger().info(f"📊 Obs. vector size: {observation.shape}")
    ...
    def control_loop(self):
        ...
        try:
            ...
            # Log any unexpected exception in the control loop
