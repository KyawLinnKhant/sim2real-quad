...
class QuadrupedController(Node):
    def __init__(self):
        ...
        # Confirm node startup and active topics
        self.get_logger().info(f'Quadruped Controller active on {self.serial_port}')
        self.get_logger().info(f'Registered joints: {list(self.joint_to_channel.keys())}')
        self.get_logger().info(f'Subscribed to /joint_states')
        self.get_logger().info(f'Test topic: /quadruped_joint_cmd')
    ...
    def connect_to_arduino(self):
        ...
        try:
            ...
            # Confirm serial link is established
            self.get_logger().info('Serial link to Arduino established.')
            
            # Test connection
            if self.send_arduino_command('echo test'):
                self.get_logger().info('Arduino comms verified.')

            # Move to default positions to match URDF
            self.get_logger().info('Applying default joint angles...')
            ...
            self.get_logger().info('Default angles applied.')
    ...
    def send_arduino_command(self, command):
        ...
        if self.serial_connection is None:
            self.get_logger().error('Serial connection unavailable.')
            return False
    ...
    def joint_state_callback(self, msg):
        ...
        for i, joint_name in enumerate(msg.name):
            if joint_name in self.joint_to_channel and i < len(msg.position):
                ...
                # Log the URDF-to-servo angle translation per joint
    ...
    def move_joint(self, joint_name, angle_degrees):
        ...
        if joint_name not in self.joint_to_channel:
            self.get_logger().error(f'Unrecognized joint: {joint_name}')
            return False
        ...
        if success:
            ...
            # Confirm joint moved and log the channel and final angle
            self.get_logger().info(f'{joint_name} (ch{channel}) -> {angle_degrees}°')
    ...
    def destroy_node(self):
        ...
        if self.serial_connection:
            # Notify before closing the serial port
            self.get_logger().info('Closing serial connection to Arduino.')
