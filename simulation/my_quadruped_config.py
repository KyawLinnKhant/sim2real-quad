...
# Defines the quadruped robot's training environment and scene setup in Isaac Lab
...
# Robot articulation config — defines spawn path, initial pose, and actuator properties
MY_QUADRUPED_CFG = ArticulationCfg(
    prim_path="{ENV_REGEX_NS}/Robot",
    spawn=sim_utils.UsdFileCfg(
        # Path to the robot's USD asset file
        usd_path="./simulation/assets/quad5.usd",
        activate_contact_sensors=True,
    ),
    # Initial joint angles tuned for a stable standing pose
...
class MyQuadrupedEnvCfg(ManagerBasedRLEnvCfg):
    ...
    # Scene layout: robot, flat terrain, and lighting
    ...
    # 72D observation space — kept consistent with the deployment observer
    observations = {
        "policy": {
    ...
    # Reward terms encouraging stable forward locomotion
    rewards = {
